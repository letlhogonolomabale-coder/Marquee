...
router.get('/users/:id/id-image', (req, res) => {
  const user = db.prepare('SELECT id_file_path FROM users WHERE id = ?').get(req.params.id);
...
router.post('/users/:id/verify', (req, res) => {
  const result = db.prepare("UPDATE users SET verification_status = 'verified' WHERE id = ?").run(req.params.id);
...
router.post('/users/:id/reject', (req, res) => {
  const result = db.prepare("UPDATE users SET verification_status = 'rejected' WHERE id = ?").run(req.params.id);
  if (result.changes === 0) return res.status(404).json({ error: 'User not found.' });
...
router.delete('/users/:id', (req, res) => {
  ...
  const user = db.prepare('SELECT id_file_path FROM users WHERE id = ?').get(targetId);
